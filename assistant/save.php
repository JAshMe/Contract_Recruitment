...
foreach ($_POST['dept'] as $dep) {
	if($emp=='n'||$emp_code=="")
	{
		if($dep=="Physics"||$dep=="Chemistry"||$dep=="GIS [Geographic Information System] Cell"||$dep=="Biotechnology")
		$misc->palert("Department selected is not valid","home.php?val=perinfo");
	}
}
...
$r=$db->process_query($query);
foreach ($_POST['dept'] as $dep) {
	 $q="INSERT INTO `department`(`user_id`, `dept`) VALUES ('$id','$dep')";
	$q = $db->process_query($q);
}
